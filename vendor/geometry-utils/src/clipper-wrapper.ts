...
export default class ClipperWrapper {
  ...
  public generateTree(
    ...
    if (useHoles) {
      // If using holes then internal polygons are read as a void within their
      // containing shape.
  ...
  public static getFinalNfps(
    ...
    if (
      !clipper.execute(ClipType.DIFFERENCE, finalNfp, PolyFillType.NON_ZERO)
    ) {
      return null;
    }
