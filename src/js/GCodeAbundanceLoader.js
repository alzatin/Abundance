...
class GCodeLoader extends Loader {
  ...
  parse(data) {
    ...
    // Cutting (tool engaged)
    const pathMaterial = new LineBasicMaterial({ color: 0x0000ff });
    pathMaterial.name = "path";

    // Extruding (printing)
    const extrudingMaterial = new LineBasicMaterial({ color: 0x000000 });
    ...
    // For CAM: differentiate between extruding (printing), cutting (path), and rapid (move/plunge)
    let maxZ = null;
    // Find max Z value in G-code
    const linesForMaxZ = data.replace(/;.+/g, "").split("\n");
    for (let i = 0; i < linesForMaxZ.length; i++) {
      const tokens = linesForMaxZ[i].split(" ");
      const args = {};
      ...
      if (args.z !== undefined) {
        if (maxZ === null || args.z > maxZ) maxZ = args.z;
      }
    }
    ...
    function addSegment(p1, p2) {
      ...
      // For 3D printing: extruding = printing, else for CAM:
      // - extruding: printing (green)
      // - cutting: path (red)
      // - rapid: move/plunge (blue)

      // For this example, let's treat:
      // - state.extruding === true: extrudingMaterial (printing)
      // - state.extruding === false && (p1.z !== p2.z): rapidMaterial (plunge/rapid)
      // - state.extruding === false && (p1.z === p2.z): pathMaterial (cutting)
      // - state.extruding === false && (p1.z === p2.z) && (p1.z === maxZ): rapidMaterial (highest Z is always rapid)

      let segmentType;
      if (state.extruding) {
        segmentType = "extruding";
        currentLayer.vertex.push({ p1, p2, type: "extruding" });
      } else if (p1.z !== p2.z) {
        segmentType = "rapid";
        currentLayer.vertex.push({ p1, p2, type: "rapid" });
      } else if (p1.z === maxZ && p2.z === maxZ) {
        segmentType = "rapid";
        currentLayer.vertex.push({ p1, p2, type: "rapid" });
      } else {
        segmentType = "path";
        currentLayer.vertex.push({ p1, p2, type: "path" });
      }
    }
    ...
    function absoluteExtrusion(v1, v2) {
      const relative = state.extrusionOverride
    ...
    function addObject(vertexSegments, i) {
      // vertexSegments: array of {p1, p2, type}
      // Group by type
      const byType = { extruding: [], path: [], rapid: [] };
      vertexSegments.forEach((seg) => {
        if (seg && seg.type && byType[seg.type]) {
          byType[seg.type].push(seg);
        }
      });

      // For each type, create geometry and add to object
      Object.entries(byType).forEach(([type, segs]) => {
        ...
        let material;
        if (type === "extruding") material = extrudingMaterial;
        else if (type === "rapid") material = rapidMaterial;
        else material = pathMaterial;
        const segments = new LineSegments(geometry, material);
        segments.name = type + "_layer" + i;
        object.add(segments);
      });
    }
    ...
    // For CAM: collect all segments by type and add as separate objects
    const allSegments = [];
    for (let i = 0; i < layers.length; i++) {
      const layer = layers[i];
      // All segments are now in layer.vertex as {p1, p2, type}
      allSegments.push(...layer.vertex);
    }
    addObject(allSegments, layers.length);
