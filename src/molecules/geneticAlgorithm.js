...
export default class GeneticAlgorithm extends Atom {
  ...
  constructor(values) {
     ...
     */
    this.description =
      "Defines a new genetic algorithm which will simulate evolution to maximize the fitness function using the input population size and number of generations. Only constants which are upstream of this atom with evolve checked will be changed.";
  ...
  updateValue() {
    if (this.evolutionInProcess) {
      ...
      this.population[this.individualIndex].fitness =
        this.findIOValue("fitness function");
  ...
  breedTwo(A, B) {
    ...
    while (geneIndex < lengthOfGenome) {
      ...
      const mutationAmount = 0.1 * (maxVal - minVal) * (Math.random() - 0.5); //Mutate by at most +-.5%
  ...
  breedAndCullPopulation() {
    ...
    while (index < this.population.length - keptPopulationNumber) {
      const individualOneIndex = Math.round(
        Math.random() * (breeders.length - 1)
      );
      const individualTwoIndex = Math.round(
        Math.random() * (breeders.length - 1)
      );
      const newIndividual = this.breedTwo(
        breeders[individualOneIndex],
        breeders[individualTwoIndex]
      );
  ...
  updateConstantsList() {
    ...
    this.inputs.forEach((input) => {
      input.connectors.forEach((connector) => {
        connector.walkBackForConstants((constantObject) => {
          this.addToConstantsList(constantObject);
        });
      });
    });
  }
