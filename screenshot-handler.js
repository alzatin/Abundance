...
module.exports = async function handleScreenshots({ github, context }) {
  ...
  const targetOwner = isFromFork
    ? context.payload.pull_request.head.repo.owner.login
    : context.repo.owner;
  const targetRepo = isFromFork
  ...
  try {
    ...
    console.log(
      `Failed to create/update screenshots branch: ${branchError.message}`
    );
    return;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      screenshotUrls[
        file
      ] = `https://raw.githubusercontent.com/${targetOwner}/${targetRepo}/${branchName}/screenshots/${file}`;
    } catch (uploadError) {
